...
def graph2vector(g):
    ...
    node_attr = np.stack((node_type, posx, posy, bldg_area), 1)
...
def graph_transform(data):
    ...
    posx = torch.tensor(posx, dtype=torch.long)
    posy = torch.tensor(posy, dtype=torch.long)
    ...
    trans_data = Data(x=node_attr_onehot, edge_attr=edge_attr_onehot, edge_index = edge_idx, y1=posx, y2 = posy)
    return trans_data
...
class UrbanGraphDataset(Dataset):
    ...
    def get(self, idx):
        ...
        y = np.array( [y_n['node_type'], y_n['posx'], y_n['posy'], y_n['bldg_area']])
        data = Data(x = node_attr, edge_attr = edge_attr, edge_index=edge_index, y = y)
        return data
    ...
    def dataset_attribute_discretize(self):
        ...
        node_attr_all[:,1] = transform_to_quantile(node_attr_all[:,1], quantile_level)
        ...
        node_attr_all[:,3] = transform_to_quantile(node_attr_all[:,3], quantile_level, 4) 
        ...
        for raw_path in self.raw_paths:
            ...
            for ii in range(node_vol):
                ...
                tmp_graph1.nodes[ii]['posx'] = curr_node_attr[ii,1]
                tmp_graph1.nodes[ii]['posy'] = curr_node_attr[ii,2]
                tmp_graph1.nodes[ii]['bldg_area'] = curr_node_attr[ii,3]
